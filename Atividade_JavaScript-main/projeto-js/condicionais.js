// const idade = Number(prompt("Digite sua idade: "));

// if (idade >= 18 ) {
//     console.log("Vc é maior de idade!");
    
// }

// if (idade >= 18) {
//      console.log("Vc é maior de idade!");
    
// } else {
//      console.log("Vc é menor de idade!");
// }


// const idade = 20;
// const temDocumento = true;
// const estaNaLista = true;

// if (idade >= 18 && temDocumento && estaNaLista) {
//     console.log("Pode entrar!");
    
// } else {
//     console.log("Acesso negado!");
// }

// const temDinheiro = false;
// const temCartao = true;
// const temPix = false;


// if (temDinheiro || temCartao || temPix) {
//     console.log("Pode pagar!");
// } else {
//     console.log("Nao tem como pagar."); 
// }


// const usuario = "admin";
// const senha = "1234";

// const senhaCorreta = (senha === "1234");
// const ehAdmin = (usuario === "admin");
// const ehGerente = (usuario === "gerente");

// if ((ehAdmin && senhaCorreta) || (ehGerente && senhaCorreta)) {
//     console.log("Acesso liberado!");
    
// } else {
//     console.log("Acesso negado");
// }

// if ((ehAdmin || senhaCorreta) && senhaCorreta) {
//     console.log("Acesso liberado!");
    
// } else {
//     console.log("Acesso negado");
// }


// const usuario = prompt("Usuario: ");
// const senha = prompt("Senha:");

// const acessoAdmin = (usuario === "admin" && senha === "1234");
// const acessoUser = (usuario === "user" && senha === "senha123");

// if (acessoAdmin || acessoUser) {
//      alert("Acesso liberado!");
// } else {
//      alert("Usuario ou senha incorretos.");
// }

// const nota = 75;

// if (nota >= 80) {
//     alert("Aprovado")
// } else if(nota >= 60) {
//      alert("Recuperacao")
// } else {
//      alert("Reprovado")
// }


// const temp = Number(prompt("Temperatura atual: "));

// if (Number.isNaN(temp)) {
//      alert("Temperatura invalida!");
    
// } else if(temp < 0){
//      alert("Muito frio")
// } else if (temp >= 0 && temp <= 15){
//      alert("Frio (0 C a 15 C)")
// }else if (tmep > 15 && temp <= 25){
//      alert("Agradavel (16 C a 25 C)")
// }else if (temp > 25  && temp <= 35){
//      alert("Muito quente (Acima de 35 C)")
// }


// const nota = Number(prompt("Digite a nota (0-100):"));

// if (Number.isNaN(nota)) // ISTO nao e um numero (tratamento para o usuário) 
//      {
//      console.log("Nota inválida! Digite apenas números.")
// } else if(nota < 0) {
//      console.log("Nota não pode ser negativa.")

// } else if (nota > 100) {
//      console.log("Nota não pode ser maior que 100.")
// } else {
//      if (nota >= 80) {
//           console.log("Aprovado! Nota:", nota)
          
//      } else if(nota >= 60){
//           console.log("Recuperção! Nota:", nota)
          
//      } else {
//           console.log("Reprovado! Nota:", nota)
//      }
// }

// const nota = Number(prompt("Digite a nota (0-100):"));

// if (Number.isNaN(nota) || nota < 0 || nota > 100) {
     
// } else {
     
// }
